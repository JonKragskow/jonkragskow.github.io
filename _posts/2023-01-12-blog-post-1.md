...
Instead, we can use a formula proposed by Gerloch and McMeeking which connects elements of the total magnetic moment operator $\hat{\mu}_{J}$ to the g-tensor. Dropping the $J$ suscript, the total magnetic moment operator $\hat{\mu}_\alpha$ for a given direction $\alpha$ is analogous to the free electron magnetic moment operator

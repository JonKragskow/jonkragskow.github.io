...
The reason for this second step becomes clear when you realise that scaling $L(x)$ by $\mathcal{A} \ \ [\mathrm{1000 \ cm \ mol^{-1}}]$ gives a function with units of $[\mathrm{1000 \ cm^{2} \ mol^{-1}}] = [\mathrm{L \ mol^{-1} \ cm^{-1}}]$, i.e. common units of the linear molar extinction coefficient $\epsilon$.

To plot all of the modes together as a single spectrum, simply sum $L(x) \cdot \mathcal{A}$ for each mode; the x-axis will be wavenumber and the y-axis will be $\epsilon$.
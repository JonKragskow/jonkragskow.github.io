...
$\mathcal{A}_\mathrm{e}$ - the IR intensities are proportional to the integrated absorption of each band, **not** the peak height.
...
The area of this function is then scaled to match the *decadic* integrated absorption coefficient $\mathcal{A}$, which is defined analogously to $\mathcal{A}_\mathrm{e}$ but using a decadic (base-10) logarithm.
...
The conversion factor between napierian and decadic integrated absorption coefficients is

$$
\mathcal{A} \ \ [\mathrm{m \ mol^{-1}}] = \frac{1}{\ln{10}} \cdot \mathcal{A}_\mathrm{e} \ \ \ \ [\mathrm{m \ mol^{-1}}]
$$

Therefore, we divide Gaussian's value of $\mathcal{A}_\mathrm{e} \ \ [\mathrm{km \ mol^{-1}}]$ by $\ln(10)$ to move from napierian to decadic integrated
